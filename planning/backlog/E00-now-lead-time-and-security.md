...
- [ ] `E00-01` (risk:critical) (owner:andy) Rotate the **live Razorpay key** (`rzp_live...`) — it is in cleartext in the `.bubble` export file
- [ ] `E00-02` (risk:critical) (owner:andy) Rotate the Stripe test secret key and the 2 Bubble marketplace plugin app secrets found in the same file
- [ ] `E00-03` (risk:critical) Add `*.bubble` to `.gitignore` before the repo exists; store the export outside the repo
- [ ] `E00-04` (risk:high) (owner:andy) Check whether Bubble's **Data API** is exposed publicly; if so, disable it (Order and Child data are currently world-readable)
- [ ] `E00-05` (risk:high) (owner:andy) Tighten Bubble privacy rules as a stopgap on the live app: `Order` (currently everyone can search/view all), `Child`, `Dish_In_Order`, `Temp`
...
- [ ] `E00-12` (owner:andy) Confirm whether menu `Price` in the Excel is GST-inclusive or exclusive (cart currently adds 5% on top)
- [ ] `E00-13` (owner:andy) Verify direct access to Apple Developer account and Google Play Console independent of Bubble
- [ ] `E00-14` (owner:andy) Locate original dish images (Bubble CDN URLs die on migration); inventory what is missing
- [ ] `E00-15` (owner:andy) Export a full Bubble **data** dump (users, children, orders, dish_in_order, schools, kitchens, menus) — hand it over; the build side inspects and reports on it in `E19-04`
- [ ] `E00-17` Draft the **secret rotation policy** for Andy to approve: which keys, how often, who does it, where they are stored
- [ ] `E00-18` (owner:andy) Check whether any legacy **prepaid card / wallet balances** exist off-system for early users; if so they must be migrated as opening ledger credits (see `E16-15`)
