...
- [ ] `E16-49` (risk:high) **The Bubble import cannot insert a historical order.** `0039` enforces §4.1, where the only legal INSERT states are `draft` (admin) and `pending_payment` (system) — everything else is reached by *moving*. That is right for the product and it has no answer for `E16`, which imports ~361 legacy orders that are already `delivered` or `cancelled`. Three options, none free: **(a)** the importer walks each order through the machine, which writes fabricated `order_event` history — a delivered-at timestamp we invented, on the audit table `I2` exists to make trustworthy; **(b)** an explicit `migration` actor in `actor_type` with its own row in §4.1 permitting a direct insert into a terminal state, which is honest about what is happening and puts the exemption in the table where it can be read; **(c)** the importer disables the trigger, which is the version that gets copied into a script nobody reviews. **Recommend (b)**, with the imported rows carrying `migration_source = 'bubble_migrated'` so they are distinguishable for ever. Decide before `E16` writes orders, not during
