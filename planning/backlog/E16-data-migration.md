...
- [ ] `E16-54` (owner:andy) (risk:high) **No dish on production carries an allergen tag — `dish_allergen` is empty for all 79.** Found 2026-08-15 during the verification sweep, when both `allergen` and `dish_allergen` were empty. **The vocabulary half is now done by the web thread** — `0063_allergen_vocabulary` seeds milk, gluten, tree nut and soy, and those four are live on production. What remains is the half only the kitchen can answer: **which of the 79 dishes contain which of them.** Until that exists every dish renders with no allergen information, which reads as *"contains nothing"*. Same shape as `E16-52` (no `food_type` on any dish) and the same reason — `catalogue.sql` will not invent it. Non-negotiable #4 makes this the highest-consequence of the data gaps, because the failure mode is a child eating something they are allergic to rather than a complaint. The app-side machinery is built and tested (`useAllergenWatchlist`, the cart warnings wired in `E05-45`) and has nothing to work with. **Ledger note**: `0063` was applied to production without being recorded in `supabase_migrations.schema_migrations` — verified byte-for-byte (the four ids match the migration exactly) and recorded, so a later `db push` does not replay it. Same class as the `0060` discrepancy, opposite direction; worth the web thread knowing
- [x] `E16-55` **The 77 legacy dish photos are on production, uploaded through `admin-dish-image`.** Production had zero images and every dish rendered blank — a visible downgrade from the app parents use today. Matched on `legacy_bubble_id`, which all 79 dishes carry and every manifest entry supplies, so it is an exact join and not a name match; `matchDishes` is reused from `upload.mjs` rather than reimplemented. Verified after the fact by comparing each dish's stored `asset.checksum_sha256` against the manifest entry for **that dish's own id** — 77 matched, 0 mismatched — and by reading a rendered contact sheet of all 79 name-and-photo pairs. Driven through the Edge Function rather than `upload.mjs`'s direct service-role write, because that is now a second write path into the bucket, `asset` and `dish.image_asset_id`; `upload.mjs` is marked superseded. `public_menu` went from 0 to 117 of 119 rows carrying an `image_path`
- [ ] `E16-56` (owner:andy) **The legacy photos are 120 pixels tall, and there is no larger original.** All 82 are 120px tall (72 are 120×120). Fetching the source URL with no size parameter returns 180×120 and `?w=1200` returns identical bytes; the Bubble export is CSVs of URLs, not binaries. They are uploaded unmodified because re-encoding a thumbnail loses quality for nothing, and they reach parity with the app parents use today — but they will look soft on a modern phone. A re-shoot decision, alongside the two dishes with no photo at all (`E16-29`)
- [ ] `E16-57` **Five pairs of dishes share one photo because they look like duplicate catalogue entries.** Choco Muffin / Chocolate Muffin, Hot Choco Milk / Hot Chocolate Chocolate, Mango Shake / Mango Shake (Seasonal), Rajma Rice / Rajma With Rice Or Prantha, Strawberry Shake / Strawberry Shake (Seasonal). Each matched its own `legacy_bubble_id` and both legacy records pointed at the same file, so the images are correct — but a menu showing two near-identical entries is worth consolidating
