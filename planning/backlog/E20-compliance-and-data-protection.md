...
- [ ] `E20-26` (risk:critical) Finish or disable `E20-10` (no PII in logs or Sentry) **before** the store privacy forms are submitted. Every "not collected" answer in `docs/andy-prep/store-privacy-answers.md` assumes it, and answering a store form wrongly is a compliance problem rather than a paperwork one
