...
- [ ] `E10-19` **Edit an override from `/admin/config`.** The write half of `E10-06`. `admin-school` already accepts a `config` patch on PATCH — the three-way absent / value / `null` distinction is implemented and tested — so this is the screen, not the endpoint: a control per setting, "Remove override" wired to send `null`, and the confirmation reading `revertsTo` (which is already computed and is **not** always the platform default). The read screen says so at its foot rather than drawing a disabled Edit button on every row
- [ ] `E10-20` **Dish and menu management screens.** `tools/bulk-import` covers the bulk path — creating and updating dishes, menus, items and assignments from CSV or JSON, with a dry run — and `docs/import-format.md` documents it. What is missing is the one-off case: change a single price without preparing a file. Needs an `admin-dish` Edge Function on the shape `admin-school` already sets, plus a screen. Not on the launch path while the importer exists
