...
- [x] `E07-01` (mvp) **Gapless invoice numbering.** `0047`. From `invoice_sequence` under a row lock, taken **last** — after every check that could refuse has passed — because a Postgres sequence is non-transactional by design and a rolled-back transaction would burn a number and leave a hole nobody can explain to an auditor. Format `GB/26-27/000417`: **15 characters**, where the original schema comment's `GB/2026-27/000417` was seventeen and would have breached Rule 46's 16-character cap
- [x] `E07-02` (mvp) **The invoice itself.** `0047`/`0048`, issued inside `settle_payment`'s transaction (`D14`) so a paid order can never exist without a tax document, nor a burned number without a settlement. Seller identity **snapshotted** (§13.2 — a reprint must be byte-identical, so a wrong value cannot be quietly fixed in config), place of supply printed always per Rule 46, CGST and SGST as separate figures (`M2`), pickup codes carried onto the document (`P4`). Line descriptions carry the child's **first name only** (§4.3, `G7`) — asserted positively and negatively, since this is a document a parent forwards to an employer. Buyer name may be NULL and is never fabricated (`E07-22`, Rule 46(f)). 13 assertions
...
- [x] `E07-16` (mvp) **Financial year derived from `issued_at`** in the platform timezone — `financial_year_at()`, `0047`. April–March, so April 2026 and March 2027 both resolve to `2026-27`. Derived rather than stored, so an invoice cannot disagree with its own series
