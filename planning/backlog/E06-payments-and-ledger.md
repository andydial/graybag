...
- [ ] `E06-20` **Order detail is unreachable.** `OrderDetailScreen.cancelling`, `imageUri` and `onContactSupport` have no caller. Expected until checkout ships, recorded so it is not mistaken for a wiring bug later
