...
- [ ] `E06-30` (risk:high) **Assert the merged `AndroidManifest.xml` contains the UPI `<queries>` block at build time**, rather than assuming our config plugin is what puts it there. Static analysis during `E19-01` setup found the block is supplied by `com.razorpay:standard-core`, which `com.razorpay:checkout` pulls in at version `LATEST` — so the thing `E06-29` depends on is a floating third-party dependency we do not control. A build-time assertion turns a silent degradation to UPI collect/QR into a failed build. Pairs with `E19-08` (pin the version)
