...
- [ ] `E02-31` (risk:high) `public.get_schools()` exists in migration `0011` but is not reachable over REST (`PGRST202` on staging) — it was never granted to `anon` or never exposed. Decide: grant and move `fetchSchools` onto it, or drop it and enforce the projection with a view. An unreachable function that a migration's security rationale depends on is worse than neither
