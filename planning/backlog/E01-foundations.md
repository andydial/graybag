...
- [ ] `E01-26` (risk:high) **`Maestro (cart)` has timed out at the 45-minute job limit on every pull request today**, including the payments thread's `e06-02-create-order`. The step that hangs is *Build the staging APK*; the last Integration run to pass was a scheduled one on 2026-08-12 at 4m12s. `#43` and `#45` were both merged around it after confirming neither diff touched a mobile, Android, Gradle or Maestro file. **That is twice, and Andy's ruling stands: a required check that times out on every PR is a gate switched off by accident, and merging around it is how it stays off.** Find the cause — a Gradle distribution fetch was seen failing with `Unexpected end of file from server`, so a pinned/cached distribution is the first thing to try — and either fix it or raise the timeout deliberately. Do not merge around it a third time
