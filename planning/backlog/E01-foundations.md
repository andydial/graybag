...
- [ ] `E01-00` (owner:andy) One-off: authorise the GitHub (`gh auth login`) and Supabase CLIs on your machine — after this the build side creates and manages both
- [ ] `E01-01` Create GitHub repo, private, monorepo layout (`apps/mobile`, `apps/web`, `packages/shared`, `supabase/`)
- [ ] `E01-02` Branch protection on `main`: PR required, CI must pass, no direct pushes
- [ ] `E01-03` `CONTRIBUTING.md` + `CLAUDE.md` (project conventions for Claude Code) + `README.md`
- [ ] `E01-04` Create Supabase **staging** project in Mumbai (`ap-south-1`)
- [ ] `E01-05` Create Supabase **production** project in Mumbai (`ap-south-1`)
- [ ] `E01-06` Local dev: Docker Postgres + Supabase CLI, seed script, works fully offline
- [ ] `E01-07` (risk:high) Secrets per environment — Razorpay **test** keys in staging, **live** keys in prod, never in code, never hand-edited
- [ ] `E01-08` CI pipeline: typecheck, lint, unit tests, integration tests against a seeded ephemeral DB
...
- [ ] `E01-10` Database migration tooling + rule that every schema change ships as a reversible migration
...
- [ ] `E01-13` Seed/fixture data representing 3 schools, 1 kitchen, 3 menus, several users and dependents
- [ ] `E01-14` Deploy pipeline: merge to `main` -> staging; tagged release -> production, with manual approval
