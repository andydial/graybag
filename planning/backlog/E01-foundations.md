...
- [ ] `E01-25` (risk:medium) **`scripts/tag-mvp.mjs` and the backlog markdown disagree about five tasks, and running the script would resolve it the wrong way.** `E03-20`, `E05-16`, `E05-20` and `E05-21` carry `(mvp)` in the markdown but are absent from the script's explicit include list, so a run **strips** them from v1 — four tasks silently leaving scope. `E09-11` is the mirror case: in the list, missing the marker in markdown. Found 2026-08-11 while registering `E04-14`…`E04-20`, and pre-existing. The list is meant to be the authority (`docs/mvp-scope.md`), so either the four belong in it or their markers are wrong — **that is a scope decision, not a cleanup**, which is why this task does not fix it. Whoever rules should also make the disagreement fail CI, because "runs the script last" is not a scope process
