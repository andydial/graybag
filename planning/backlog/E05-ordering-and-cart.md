...
- [ ] `E05-38` **Surface self-ordering** (`P13`). "Order for myself" as a first-class entry in Who-to-order-for when the account has no `is_self` recipient, and Add-someone asking who it is for before anything else. `create_recipient` already takes `p_is_self` (`0022`) and `NewRecipient` needs an `isSelf` field to reach it. Removes the "am I supposed to add myself as a child" confusion
- [ ] `E05-39` **Capture the account holder's own name on the ORDER CONFIRMED screen** (`P18`). One optional field, "What should we call you?", with a clear skip; also settable from Account; never asked twice. `app_user.first_name`/`last_name` exist and are never written. **Order one must work with no name everywhere** — invoice falls back to the email local-part, packing list to the child's name. Depends on `E06` for the confirmation screen
- [ ] `E05-41` **Audit every surface that shows the account holder's name for the no-name case** (`P18`). Invoice, packing list, support view, and every email. Andy's instruction: if any of them look broken without a name, report it rather than making the field required
