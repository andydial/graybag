...
- [ ] `E05-44` **Remove a child** — `deactivate_recipient` (`0025`) and `api.removeRecipient`. Deactivates the row and revokes every guardian link, so the child leaves all guardians' lists; refuses while undelivered orders exist. NOT DPDP erasure (`E20-06`) — order history, invoices and ledger entries are retained deliberately. **Server and client only — the UI is `E05-37`**
