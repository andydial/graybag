...
- [x] `E05-36` **Cart lines show the dish photo and veg mark.** The prop existed and nothing ever passed it, so every line rendered the placeholder and the cart read as a different app from the grid. `RootNavigator` now builds `dishInfo` from the cached menu — no extra request, since it is the read the Menu tab just did
...
- [x] `E05-34` **Remove a child** — `deactivate_recipient` (`0025`) and `api.removeRecipient`. Deactivates the row and revokes every guardian link, so the child leaves all guardians' lists; refuses while undelivered orders exist. NOT DPDP erasure (`E20-06`) — order history, invoices and ledger entries are retained deliberately. **Server and client only — the UI is `E05-37`**
...
- [ ] `E05-39` **Capture the account holder's own name.** `app_user.first_name`/`last_name` exist and are never written — email OTP yields an address and nothing else, so nothing can greet a parent, the kitchen has no name against an order and support has nobody to address. **No signup step** (`AR7`): see `docs/decisions/product.md` `P18` for where it is captured instead
