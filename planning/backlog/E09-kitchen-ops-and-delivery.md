...
- [ ] `E09-04` (mvp) **The kitchen dashboard: one working list of the day's orders.** Child, class, section, break, school, dishes, status. Filters: school, date, break, status. **Production totals as a summary at the top** — "how much do we cook" is answered by the same data, so it is a header on this list rather than a second screen. `productionTotals` and `perSchoolTotals` already compute it (`E09-01`, `E09-02`)
- [ ] `E09-05` (mvp) **Update status from the list**: mark delivered (bulk per class, one tap — `L8`), mark prepared, and cancel with a reason (`E09-08`). Optimistic, failing loudly; `delivered` is terminal (`T8`/`T9`)
