...
- [ ] `E17-36` (owner:andy) **Confirm whether the live iOS listing supports iPad.** `app.json` sets `supportsTablet: false`. If the live app is iPad-enabled, this update drops a device family from under existing users. Cheap to check while in App Store Connect; low severity, unverified either way
