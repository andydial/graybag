...
- [ ] `E17-34` (risk:high) **Move the EAS Android version counter above the live `versionCode`** once `E17-33` returns — `eas build:version:set --platform android`. Fill `LIVE_PLAY` in `app-config.test.ts`, which activates the two dormant assertions. Do this *before* paying for an Android production build, not after Play rejects one
