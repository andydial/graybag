...
- [ ] `E17-30` **App Store Connect app id wired into `eas.json`.** `ascAppId` 6749555467 alongside `appleTeamId` F247T8Y2NT, on both `submit.production` and a new `submit.preview`. `preview` also became an iOS store build — it extended `staging`'s `distribution: "internal"`, which is ad-hoc and cannot reach TestFlight (`R9`)
