...
- [ ] `E03-09` (mvp) Session handling across app restarts and token expiry, tested on a cold device. **Storage landed 2026-08-10 with `E03-20`** — expo-secure-store, chunked to stay under the ~2 KB keychain warning threshold. Left open for the half that needs a device: token expiry and a genuine cold start
...
- [ ] `E03-24` Wire clipboard auto-fill on the OTP screen. The behaviour is built and tested behind an optional `readClipboard` prop (`E21-07`); it needs `expo-clipboard` as a dependency and one line in `screens/index.tsx`. A native module, so it needs a new dev-client build
- [ ] `E03-25` (risk:medium) Decide whether a pending OTP draft survives **process death**. It survives backgrounding and unmount today via an in-memory draft; persisting a live code to disk is a security call that should be made deliberately rather than by whoever touches the file next
- [ ] `E03-26` (risk:critical) **One source of truth for "am I signed in".** `SessionContext` started `signedOut` and was never restored, while the Supabase client restored its own persisted session — so on every cold start the app held two answers and they disagreed. Home offered "Add someone to place an order", the cart rendered a line for a named child, and Account offered Sign in, all at once. A minor's first name on screen with no session is a DPDP tier-P disclosure (non-negotiable #4), not a cosmetic bug. Fixed by deriving the session from the client's stored session, adding a third `unknown` state so no screen claims an answer it has not checked, deriving the order target from the session (cleared on sign-out, not merely left stale), and routing "Add someone" to sign-in first for a visitor. Guarded by `src/session/no-recipient-without-session.test.tsx`, whose structural half fails the build if any screen reads the recipient without proof of a session
