...
- [ ] `E03-11` (risk:critical) (mvp) ~~Migration path for the ~400 existing users: match on **normalised E.164** mobile number~~ — **superseded by `E03-16`**: migration now matches on **email** per `U1` (the legacy phone `number` field lost leading zeros/`+91`, an account-takeover vector). Kept for history; do not build the phone-match path
