...
- [ ] `E09-11` _(fast-follow)_ (risk:high) Decide whether the kitchen packing list and per-class delivery sheet surface a parent's per-line note. If they don't, the note field is not built at all — a note nobody reads is a lie told to a parent trying to be careful (ux-spec §5.6.1)
