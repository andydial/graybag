...
| 22 | **Kitchen shows who ordered — and `0095`'s guard refused every Super Admin** | 2026-09-18 | **Emergency. Shipped `0095`, which was wrong, then `0096` which fixes it.** Andy saw *"Ordered by — not shown"* on every card while signed in as Super Admin. **Confirmed from production data before changing anything**, by asking the view as each real account rather than reading the code: `ANDY (orders.view_pii @ platform) -> 0 rows`, `VIVEK (orders.view_pii @ kitchen) -> 43 rows`, `NOBODY -> 0 rows`. **The cause was that `0095` reimplemented authorization instead of calling it** — a hand-rolled `EXISTS` over `permission_grant` testing only `school` and `kitchen` scopes. `auth_has_permission` already implements platform-satisfies-any-scope, the **platform owner who holds no grant rows at all** (`E02-39`, a second and independent reason the copy would have failed), and city/kitchen → school inheritance. The copy reproduced two branches and dropped the rest. `0096` replaces the whole thing with `auth_can('orders.view_pii','school',o.school_id)` — the same predicate shape as `order_read_backoffice`, so the email follows exactly the orders the caller can already see. **`authorization.test.sql` §12 could not have caught it**: it asserts the view's *shape* — definer, restates `auth_is_live_user`, two columns, guard in its own WHERE — and every one of those passed while the feature showed nothing to anybody who mattered. A shape is not a behaviour. `kitchen_order_contact.test.sql` now asks the view as three real accounts; it fails **exactly the two platform assertions** against `0095` and passes 9/9 against `0096`, checked in that order. **Migration number contested**: `0096` was also claimed by PR #196, whose own migrations suite was failing, and every number above `0095` was a *gap* — which `check-migrations` forbids as firmly as a duplicate. Taken deliberately; #196 renumbers on its next rebase and the checker will say so |
...
| 27 | **The re-walk failed the same way: which bundle was running, and why the fix did not work** | 2026-09-18 | **Bundle settled first, as you asked, and then the real cause — which is mine.** **Which bundle:** the `checkout` call at 02:07:50 UTC is followed straight by `checkout-status` polls with **no `payments-create-order` request at all**, which is `E21-98` behaving exactly as designed — a pre-fix bundle would have called it and taken a 409. So it was not an old bundle. **I cannot distinguish the last two OTAs from the server, and it does not matter**: the only client change between them was `E21-102`'s refresh, whose call site is gated behind the terminal-status check that never became reachable, so it could not have run whichever bundle you had. The failure was **entirely server-side**. **Your order:** `GB-0RH40X`, placed 12:07:45 your time, and a second at 12:18 (`GB-7A0ER1`). Both: `order.status` **`paid`** with a pickup code, `payable_paise` 0, `pack_applied_paise` set, `paid_at` set — and `order_group.status` **`draft`**, from the moment of creation and never updated. **Why `0094` did not fire:** its rule required `paid_at is not null`, and `create_checkout` confirms the orders **first** and writes `paid_at` one statement **later**; the derivation is a trigger on `order`, so it read the stamp before it existed. **A trigger sees statements, not transactions** — and `0094`'s comment asserted the opposite. Every `now()` on those rows is identical, which is exactly why the data looks like proof the writes were simultaneous. **The test is the worse half and you should know it**: `meal_packs.test.sql` §16 stamped `paid_at` at insert time and *then* moved the order — the real sequence reversed — so it was green against broken code; and because it stamped first, the rule fired over a pending order, an assertion failed, and I added a condition **to satisfy a fixture that was itself wrong**, then reported that as the test catching a hole. It was the opposite. The happy path now lives in `checkout.test.sql` and **calls `create_checkout`**, so there is no write order for a fixture to get wrong. Mutation-checked both ways: the new test fails against the rule that shipped, the old one passes. **And the deeper problem:** the Account screen has shown `OTA <7 chars>` since `E17-54` — it was already there and I should have told you where — but it named the bundle without saying whether it was current, so matching it still needed me. It now reads `Staging · 6a0f088 · OTA 01a0b25, published 18 Sept, 19:41`. Staging is at **`0096`** with all three orders reconciled to `paid`, OTA **`01a0b25`** serving `appEnv=staging` on both platforms. **One thing found on the way out and it is worth your knowing:** the web thread took `0095` for the kitchen-email view and merged first, so mine renumbered to `0096` — but I had already applied and recorded mine as `0095` on staging, so staging's ledger claimed `0095` was done while the SQL that ran under that number was mine. A `db push` would have **skipped their migration silently**. Reconciled after verifying both halves, and **their `0095` is still unapplied on staging** — theirs to deploy, not mine to run |
| 26 | **Three from the second staging walk: the cart, the balance, and the email copy** | 2026-09-17 | **Added 2026-09-17 and cleared the same day. The redemption itself worked** — order placed, covered correctly, kitchen got it, delivery email arrived. **(1) The cart did not empty**, and you asked the right question with it. It is **not** a pack-specific path and the ₹0 route does **not** skip the clear step: it reaches exactly the step a cash order reaches and can never satisfy it. `derive_order_group_status` derives the group from **captured money**, and a redemption captures none — so `GB-947XS0`, whose order was `delivered` with a pickup code, had a group still reading **`draft`**. `checkout-status` answers from that field, said `unpaid` for ever, and the app polled indefinitely: no confirmation, no clear. Fixed in the function that owns the field rather than in `create_checkout`, because `L1` forbids a second writer — one new case guarded four ways, every guard proved load-bearing by removing it, and one of those guards exists **because the test caught its absence**. **(2) The balance** now refreshes on redemption as well as purchase, at the confirmation and unconditionally — gating it on the client's own coverage view would skip the refresh in exactly the case where the client was wrong. **(3) The email** drops the time entirely, as you asked; the formatter went with it rather than being left uncalled, and what to remember if a time is ever wanted back is written where it used to be. Sign-off is now *"Thanks for using GrayBag."* The whole email is pinned as one exact string, so the shape cannot drift a line at a time. Staging is at **`0094`** with `GB-947XS0` reconciled to `paid`, and OTA `42db94f4` is serving `appEnv=staging` on both platforms |
| 25 | **Tell the parent when the food is delivered** | 2026-09-17 | **Added 2026-09-17 — new, and it went to the bottom of the queue as the rules require, then was taken immediately because item 24 was already in flight in the same files.** Andy: *"parents should hear that their child got the food, and right now we tell them nothing."* He asked the question first and the answer was plain: **nothing was sent** — `kitchen-order-status` is the only thing that marks an order delivered and its one email call sat inside the `cancelled` branch, so delivering wrote `delivered_at`, `delivered_by_user_id` and an `order_event` row and told nobody. `E21-99` sends it, **alongside** that branch rather than inside it, so neither email inherits the other's guards. **Exactly once, twice over**: the caller passes `result.updated`, which holds only orders whose status actually moved — the rows are locked `for update` before that split, so a second tablet pressing *Mark all delivered* blocks and then finds nothing — and `uq_notification_one_per_order` is the independent backstop inside the sender. That is *"key it on the state change, not the button"* enforced in both places, and it needed **no migration**. It cannot fail the delivery: every path returns an outcome, the `catch` is total, and the caller ignores the result. **Always a row**, including when the send is never attempted — `E21-93`'s rule applied from the start rather than retrofitted. Content is the child's name, the items with quantities, the school, the break and the time delivered, **and no prices** — it is a receipt for food, not for money. **The name is in the email and nowhere else**: not in `notification_delivery` (whose `0001` comment names this exact email as the reason that rule exists), not in the subject line, not in a log. Bulk is one call per order, so thirty children is thirty emails. Your two required tests are the first two in the file and both fail against the real defect. **One thing found on the way and fixed**: `E21-100`, the cancellation notice was keying its dedup row on the order **group**, and a cart spanning several days is one group with one order per day — so cancelling two days would have sent one email and reported two. Never fired; production has no multi-order groups |
| 24 | **Covered items were still being charged, and three more from the staging walk** | 2026-09-17 | **All four diagnosed from logs and data before anything changed, and fixed.** **(1) The money.** The strip said *"Nothing to pay"* and the summary beneath it said ₹260.42, because the cart had **two expressions for one number** — `coverCart` fed the strip, `gstBreakdown(cart.lines)` fed the totals, the button and the figure sent to the server, and the second had never heard of the pack. **Nobody was charged**: `L7` compares what the app displayed against what the server priced and refused, and Postgres' own log says so — `pack coverage changed: expected 26042, server says 0`. That guard is the only thing between this and a parent paying twice for the same food, so the fix makes the app tell the truth rather than softening the guard. One expression now, `cashBreakdown(coverage)`, proved equal to the old one **to the paise** on every ordinary cart, because a single paise of disagreement is every checkout failing with `price_changed`. **(2) Place order failing was a consequence**, confirmed from that same log line rather than assumed. **(3) The button** now says `Place order · Nothing to pay`; with the totals fixed it would otherwise read `₹0.00`, which is accurate and still wrong — ₹0.00 is a price, and this is a redemption of something already bought and already taxed. **(4) A fourth fault, latent behind the first**: `create_checkout` returns `status: 'paid'` when a pack covers everything — it confirms the redemptions, allocates the pickup codes, sets `paid_at` and emails inline — and **nothing read that field**, so the app went on to ask Razorpay for an order with nothing owing and would have shown a placed, paid, confirmed order to the parent as a failure (`E21-98`). **Orphaned unpaid orders — the lifecycle you asked for before I change anything.** Two different things. **The pack reservation is handled**: `release_dead_meal_pack_reservations` frees a `held` redemption when its payment row is `failed` or the hold is over 24 hours old, deliberately conservative because releasing early means a payment in flight settles against a balance that has gone; it runs daily from `ops-heartbeat`. **The order has no lifecycle at all**, and that is already `E05-51`, open, `risk:high` — the enum still reads `draft, pending_payment, paid, preparing, delivered, cancelled, refunded` with no `expired`, checked on staging rather than trusted from the ticket. *"Finish paying"* is `E05-54`, which made the order **payable**, not closable. Your specific orders hold **nothing**: `meal_pack_redemption` has zero rows ever and the pack reads `items_reserved = 0` — and the reason is worse rather than better, since no pack-covered checkout has ever succeeded. The tail that matters is that `deactivate_recipient` and `change_recipient_school` both refuse while an undelivered order exists, so an abandoned order permanently blocks deleting that child. **Not changed — the mechanism is still an open decision on `E05-51`** |
