...
| 13 | **Fix the production deploy workflow, apply `0068`–`0075`, and list what else is unapplied** | 2026-08-28 | **Added 2026-08-28.** Workflow fixed (`E17-63`, PR #152): the 25 Aug run did not fail on the Migrations job — that job never began. It was a `workflow_dispatch` from `main`, and the `production` environment admits `v*` **tags** only, so GitHub refused the ref at the gate: one second, zero steps, no logs. **No `v*` tag has ever existed either, so the workflow has never once succeeded** — production's 67 migrations and 20 Edge Functions were all applied by hand. Applying via tag `v2026.08.28`, which is the only path the environment accepts and which pauses on Andy's required-reviewer approval. Besides the eight migrations, **nine Edge Functions are behind `main`**: `admin-pack-offer` and `confirm-pack-plan` never deployed; `admin-dish`, `admin-dish-image`, `admin-grants`, `admin-import`, `enquiry-submit` stale on their own code; `payments-drain` and `payments-webhook` stale via `_shared/money-alert.ts`, so **`E15-12`'s money alerting is not live on production** |
