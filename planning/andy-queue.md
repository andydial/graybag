...
| 20 | **Meal packs, rebuilt — the admin, the school switch, pack-money reporting, the GST invoice** | 2026-09-16 | **Everything buildable is merged to `main`. NOT promoted, and the reason is evidence rather than caution.** Production lacks the rebuilt schema — `items_count` returns **400**, `meals_count` returns **200**, `meal_pack_money` returns **404** — so promoting would turn `/admin/packs` from a working empty state into an error state. MOBILE's `0085`–`0087` are applied to the shared **local** database and are on no branch, so there is nothing to promote behind. **Shipped:** offer CRUD for the item model (`items_count`, `bonus_items_count`, `bonus_window_days`), with `FROZEN_ONCE_SOLD` now **empty** because every term including the name is stamped at sale — the `E21-67` fix; the per-school switch **moved to `/admin/schools`** and removed from `/admin/packs`, read-only without `meal_packs.manage` at platform scope, reversing `E10-71` with the reversal recorded beside it; pack money as its own section on `/admin/sales` — sold, GST, deferred outstanding, recognised, breakage on its own column, redemption rate per offer, bonus granted and redeemed separately — never merged into the food revenue line (`M10`, `M11`, `M12`, `M13`); the kitchen indistinguishability suite in **both** directions; and the no-enabled-school suite including a **structural** assertion that fails when the pack module grows an unreviewed entry point. **Three things the work itself corrected:** `deferredPaise` divided by `items_total`, which under Andy's settled ruling re-defers revenue recognised in an earlier month; a redundant `return 0` made that denominator unreachable to its own test, so removing it took the mutation from 1 failing assertion to 3; and the report's blind state asserted *"packs have been sold"* in a case where nothing had been, which is what production would have shown. **Blocked on MOBILE for six things**, all recorded above as requirements — the last of which, `issue_invoice` still reading `mp.meals_total`, is a **money-path defect** that fails on the first real pack purchase |
...
| 21 | **Replace the meal packs implementation** | 2026-09-16 | **Added 2026-09-16, and added to the bottom as the rules require — it is new work, not a continuation of items 10 and 12.** It supersedes both of those: the parent-side packs work (item 10) and `E21-48`/`E21-44` (item 12) are building the design Andy has now ruled wrong, so they close unbuilt rather than merging. **Design posted for review, no code written** — `docs/meal-packs-rebuild.md`, which supersedes `docs/meal-packs-plan.md`. **The premise was checked before anything was proposed for deletion and it is wrong in one place**: `meal_packs_confirmed` is **`true`** on production, not `false` as the brief said, so the guard that stops an offer going live has been **off**. It has cost nothing — zero offers, zero packs, zero redemptions, zero deferred-revenue ledger entries, zero `meal_pack_purchase` groups — so the conclusion that drove the instruction holds: there is no live pack data to preserve, and I am deleting on the row counts rather than the flag. **Blocked on five answers** (§12): the bonus grant restates revenue already recognised, possibly across a month boundary, which is what Andy's own "per-item value drops accordingly" arithmetically requires; Pack 2's bonus item count is missing from the brief; two readings of the per-school switch; most-expensive-first coverage; and whether cancelling a pack-covered order returns the item. Four of the five change the schema. **Two corrections from Andy the same day are folded in**: "This Week" falls back to the featured dish rather than showing nothing, and the live ordering path — cart, checkout, webhook, order confirmation — is **covered by a characterisation suite before it is touched**, which makes that suite the first commit rather than pack code |
...
### 6. **`issue_invoice` is broken against the new schema — a pack purchase cannot be invoiced**

**Found 2026-09-16 by running the expression, not by reading it. This is a money-path defect and
it is why the web thread stopped rather than carrying on.**

`issue_invoice` still builds the pack purchase's invoice line from the old model:
...
`meal_pack.meals_total` **does not exist** in the schema you applied locally. Proven against it
rather than inferred:

```
$ select mp.meals_total from meal_pack mp limit 1;
ERROR:  column mp.meals_total does not exist
```

PL/pgSQL does not resolve column references until execution, so this is stored happily and fails
on the **first real pack purchase**. Either the settlement transaction aborts — money captured, the
pack never activated — or the invoice is skipped and GST has been collected with no tax invoice
issued. Both are worse than a screen being wrong.

Two things to fix in the same edit, because the second is already paid for:

1. `mp.meals_total` → `mp.items_original` (or whatever the line should say the parent bought).
2. **`o.name` → `mp.name_snapshot`.** The join to the live offer is `E21-67` still present in this
   function: rename an offer between sale and settlement and the invoice is issued under a name the
   pack was never sold under. `name_snapshot` exists precisely for this and costs one word here.
   The join to `meal_pack_offer` then disappears entirely.

This is in **uncommitted local work** — `0085`–`0087` are applied to the shared database and are on
no branch — so it may already be on your list. Recorded because the failure mode is silent until
the first parent pays, and because non-negotiable #4's neighbour applies: an invoice is a statutory
document, and *"a pack purchase with no pack cannot be invoiced"* is the only guard in that branch.
