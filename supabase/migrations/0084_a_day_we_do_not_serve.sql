-- =============================================================================
-- `E05-55`. A day the school does not serve is told the truth about itself.
--
-- `create_checkout` had **no school service-day check at all**. A Sunday was refused only because
-- no menu item lists Sunday in `available_days`, so it came out as
-- `{"code":"unavailable","message":"One of the dishes is no longer on the menu for that day."}`.
--
-- Two facts arrived as one sentence — *this school does not serve that day* and *this dish came
-- off the menu* — and the parent was shown the wrong one, so they change the dish and fail
-- identically. §5.21's collapsed-states defect, where it costs an order.
--
-- Verified on production 2026-08-16, and again by a real parent on Sunday 2026-08-30.
--
-- ## The body is production's, with one block inserted
--
-- `create_checkout` is 328 lines and takes money, so this is not a re-derivation. The body is
-- `pg_proc.prosrc` read from production, verified byte-identical to what a local `db reset`
-- composes from `0014` -> `0019` -> `0039` -> `0045` -> `0060`. Exactly one guard is added, and
-- **before** the menu-item join, because that is where the wrong refusal is raised — a check
-- after it would never be reached on the day it matters.
--
-- The signature is read from `pg_proc`, not inferred: `returns jsonb`, plain `language plpgsql`,
-- **not** `security definer` and **no** `search_path`. A first draft guessed `returns uuid` with
-- both of those added; Postgres refused on the return type (`42P13`), which is the only reason
-- the accidental privilege change was caught before it shipped. It runs as its caller —
-- `service_role`, from the `checkout` Edge Function — and that is deliberate.
-- =============================================================================
...
comment on function create_checkout(uuid, text, text, bigint, jsonb) is
  ...
  'menu-item weekday rule, so a non-service day reports not_a_service_day rather than blaming a '
  'dish. The advisory calendar is orderable_calendar (E05-52); this is the authoritative refusal.';
