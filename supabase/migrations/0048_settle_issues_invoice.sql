-- =============================================================================
-- 0048_settle_issues_invoice.sql — settlement issues the receipt. `D14`, `E07-01`.
-- =============================================================================
--
-- `0047` built the issuer; this is the one line that makes it part of settling.
--
-- **In the same transaction, deliberately.** `D14`: a paid order with no invoice is a customer
-- charged with no tax document. An invoice with no settlement is a number burned out of a
-- gapless series that cannot be explained to an auditor. Either failure is unrepairable after
-- the fact, so both happen or neither does.
--
-- It runs **after** the orders are paid and their pickup codes allocated, because
-- `invoice.pickup_codes` collects them (`P4` — the code is printed on the invoice as well as
-- emailed, so a child without a phone can still collect).
--
-- Regenerated from `pg_get_functiondef()` with one `perform` inserted and asserted to differ in
-- nothing else, the same discipline every other function change in this epic has used.
-- =============================================================================
