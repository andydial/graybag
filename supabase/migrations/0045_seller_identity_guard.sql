-- =============================================================================
-- 0045_seller_identity_guard.sql — production cannot take money it cannot invoice. `E07-20`.
-- =============================================================================
--
-- `E07-20` (risk:critical, MVP) asks for `POST /checkout` to refuse in production while
-- `seller_gstin` or `sac_code` is still a placeholder. Building it found the reason it matters:
--
-- **the seller's identity has no home in configuration at all.** `seller_gstin`,
-- `seller_legal_name` and `seller_address` exist only as snapshot columns on `invoice`, which
-- means the invoice builder has nowhere to read them from. `sac_code` is on `platform_config`;
-- the other three were never added.
--
-- ## Why this is the shape of the failure, not just a missing column
--
-- `E07-20`'s own text spells out what happens without the guard, and it is the worst sequence in
-- the product: under auto-capture (`[OL-01]`) the customer is **already charged**, then
-- `settle_payment()` cannot allocate an invoice number, the settlement transaction rolls back,
-- `PY2` returns `200`, and our own sweep retries the failure for ever. Every customer charged, no
-- order created, no 5xx, no alert — and a growing `payment_webhook_event` backlog as the only
-- trace.
--
-- So the refusal belongs **before the money moves**, which is checkout. `E07-13`'s refusal inside
-- `settle_payment()` stays as defence in depth; it is the one that fires after the charge.
--
-- ## The placeholder convention is the one already in use
--
-- `«ANGLE-BRACKET»` tokens, exactly as the policy documents use them (`E20-22` already fails a
-- production build on one). A value matching `^«.*»$` is unset — not empty, not null, but
-- *visibly* unset, which is what makes an unfinished config legible in a database dump rather
-- than looking like a real short string.
--
-- ## `environment`, and why it defaults to `local`
--
-- There was no environment marker in the database. Without one this guard either fires nowhere or
-- fires everywhere, and firing everywhere would mean no checkout works on any developer machine
-- until the real GSTIN is known — which would be a guard people switch off.
--
-- It defaults to `'local'`, so **the safe direction is the default**: a new database is a
-- developer's, and staging or production has to say so deliberately. `E06-14`'s
-- environment-scoped key check has somewhere to key off now too.
-- =============================================================================

alter table platform_config
  add column if not exists environment text not null default 'local',
  add column if not exists seller_gstin text not null default '«GRAYBAG-GSTIN-PENDING-E00-10»',
  add column if not exists seller_legal_name text not null
    default '«GRAYBAG-LEGAL-ENTITY-NAME-PENDING-E20-01»',
  add column if not exists seller_address text not null
    default '«GRAYBAG-REGISTERED-ADDRESS-PENDING-E20-01»';

-- Dropped first so a re-run is a no-op: `add constraint` has no IF NOT EXISTS.
alter table platform_config drop constraint if exists platform_config_environment_known;
alter table platform_config add constraint platform_config_environment_known
  check (environment in ('local', 'staging', 'production'));

comment on column platform_config.environment is
  'local | staging | production. Defaults to `local` so the SAFE direction is the default: a new database is a developer''s, and an environment that enforces production rules has to say so deliberately. E07-20 reads it; E06-14''s key-prefix check will too.';

comment on column platform_config.seller_gstin is
  'GrayBag''s own GSTIN, snapshotted onto every invoice. A value matching ^«.*» is UNSET — visibly so, which is what makes an unfinished config legible in a dump rather than looking like a real short string. E00-10 supplies the real one; until then production cannot take an order (E07-20).';

-- -----------------------------------------------------------------------------
-- Is the seller identity real?
--
-- One function, so checkout, `settle_payment` (`E07-13`) and any boot assertion all ask the same
-- question and cannot drift on what "configured" means.
-- -----------------------------------------------------------------------------
create or replace function seller_identity_placeholders() returns text[]
language sql
stable
security definer
...
as $$
  select coalesce(array_agg(field order by field), '{}'::text[])
    from (
      select 'seller_gstin' as field, seller_gstin as value from platform_config where id = 1
      union all
      select 'seller_legal_name', seller_legal_name from platform_config where id = 1
      union all
      select 'seller_address', seller_address from platform_config where id = 1
      union all
      select 'sac_code', sac_code from platform_config where id = 1
    ) f
   where value ~ '^«.*»$';
$$;

comment on function seller_identity_placeholders() is
  'Which parts of the seller identity are still placeholders, as a sorted array — empty means configured. ONE definition of "configured" so checkout, settle_payment (E07-13) and any boot assertion cannot drift. E07-20.';

create or replace function assert_seller_identity_configured() returns void
language plpgsql
stable
security definer
set search_path = public, pg_temp
...
declare
  ...
  v_env     text;
begin
  select environment into v_env from platform_config where id = 1;

  -- Only production refuses. Elsewhere an unconfigured seller is the ordinary state of a
  ...
  v_missing := seller_identity_placeholders();
  if array_length(v_missing, 1) > 0 then
    -- Names the fields. "Configuration is incomplete" sends somebody to read four columns.
    raise exception 'seller identity is not configured: %', array_to_string(v_missing, ', ')
      using errcode = 'P0001', hint = 'seller_identity_placeholder';
  end if;
end;
$$;

comment on function assert_seller_identity_configured() is
  'E07-20: refuses in PRODUCTION while any part of the seller identity is a «placeholder». Called by create_checkout BEFORE money moves — because without it, under auto-capture the customer is charged, settle_payment cannot allocate an invoice number, the settlement rolls back, PY2 returns 200 and the sweep retries for ever: every customer charged, no order created, no 5xx and no alert. E07-13''s refusal inside settle_payment stays as defence in depth; it is the one that fires after the charge.';

-- -----------------------------------------------------------------------------
-- `create_checkout` asks the question before it takes the money.
--
-- Regenerated from `pg_get_functiondef()` with one `perform` inserted, asserted to differ in
-- nothing else — the same discipline every function change in this epic has used.
-- -----------------------------------------------------------------------------
