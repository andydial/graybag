...
-- and the last valued item always lands on exactly zero. See `M11`.
--
-- **Bonus items carry no value** (`M10`, Andy's ruling). `items_original` is the denominator for
...
create table meal_pack (
  ...
  -- BONUS items: a giveaway worth nothing in the books (`M10`).
...
comment on column meal_pack.items_original is
  ...
  'grow when the bonus is granted: bonus items carry no value (M10).';
