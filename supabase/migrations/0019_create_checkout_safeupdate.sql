-- =============================================================================
-- 0019_create_checkout_safeupdate.sql
--
-- **Every real order failed on staging with `21000: DELETE requires a WHERE clause`.**
-- `E05-21`.
-- =============================================================================
--
-- `create_checkout` clears its per-call temp table with an unqualified
-- `delete from tmp_checkout_lines;`. Hosted Supabase loads **`safeupdate`**, which rejects a
-- DELETE with no WHERE; a local `supabase start` does not. So the statement passed all 24
-- pgTAP assertions, on every run, and failed on the hosted project for every order placed
-- through PostgREST or an Edge Function.
--
-- The class is worth naming, because it is the third of its kind found today: **a check that
-- exists only in the deployed environment.** `E05-16` was two layers each testing against
-- fixtures the other never produced; `E05-20` was a precondition every fixture supplied and
-- nothing created. This one is a guard the local stack does not run at all. None of the three
-- is findable by adding assertions to the layer that looks wrong — only by exercising the
-- real path against the real environment, which is what `scripts/order-path-check.mjs` does.
--
-- The whole function is restated because `create or replace` takes a body, not a patch. The
-- only change is the `where true` and the comment above it.
-- =============================================================================
