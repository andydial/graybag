-- `issue_invoice` can invoice a pack purchase again. `E21-82`, risk:critical.
--
-- **Found by WEB's audit, and it is mine.** `0085` renamed four columns on `meal_pack` and I
-- replaced `settle_payment` without replacing the function it calls one line later.
-- `issue_invoice` still read:
--
--     mp.net_price_paise   ->  gone, now price_paid_paise
--     mp.tax_total_paise   ->  gone, now cgst_paise + sgst_paise
--     mp.meals_total       ->  gone, now items_original
--     join meal_pack_offer o on o.id = mp.offer_id, for o.name
--
-- WEB found the third; there were four. The first aborts the statement before the description is
-- ever reached, which is why the symptom they predicted understates it.
--
-- ## Why this had to be fixed before anything else
--
-- **PL/pgSQL does not resolve column references until execution**, so a body naming a column that
-- does not exist is stored happily and fails on the first call. `issue_invoice` is called by
-- `settle_payment`, inside the transaction that turns a capture into a paid order — so the first
-- real pack purchase would have hit `column mp.net_price_paise does not exist` and **rolled the
-- whole settlement back with the money already captured at Razorpay**: a parent charged Rs 3,150,
-- no pack, no invoice, and a webhook retrying into the same failure for ever.
--
-- That is the worst state this system can reach, and `D14` names it: a paid group with no invoice
-- is a customer charged with no tax document, and neither half is repairable afterwards.
--
-- **It never fired.** Every offer is `is_active = false` and no `meal_pack_offer_school` row
-- exists, so no pack has ever been bought on production — the two acts that hold packs dark are
-- also what kept this latent. Fixed before either is turned.
--
-- ## The fourth change is `E21-67` again, in the one place it cannot be undone
--
-- The description joined the LIVE offer for its name. Renaming an offer between a sale and its
-- settlement would issue the tax invoice under a name the pack was never sold under, and
-- `invoice_line` is written once. `name_snapshot` removes the join entirely.
--
-- ## Everything else is byte-identical
--
-- The body below is `pg_proc.prosrc` read back from the database rather than re-derived, with
-- exactly those four references changed. The food branch, the gapless sequence, the buyer
-- snapshot and `D14`'s in-transaction guarantee are untouched.
--
-- irreversible: a `create or replace` of a live function. The previous body is in git and in
-- `0079`, and restoring it would reinstate a settlement that aborts on the first pack sale.

begin;

CREATE OR REPLACE FUNCTION public.issue_invoice(p_order_group_id uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if v_group.kind = 'meal_pack_purchase' then
    select mp.price_paid_paise, mp.cgst_paise, mp.sgst_paise
  ...
  if v_group.kind = 'meal_pack_purchase' then
    ...
    insert into invoice_line (invoice_id, line_no, order_line_id, description, sac_code, quantity,
                              ...
                              total_paise)
    select v_id, 1, null,
           -- `name_snapshot`, and NO join to the live offer. E21-67: renaming an offer between
           -- the sale and the settlement would otherwise issue the tax invoice under a name the
           -- pack was never sold under, and an issued invoice cannot be rewritten.
           mp.name_snapshot || ' — ' || mp.items_original || ' items, prepaid',
           v_cfg.sac_code, 1, mp.price_paid_paise, mp.price_paid_paise,
           mp.cgst_paise, mp.sgst_paise, mp.price_paid_paise + mp.cgst_paise + mp.sgst_paise
      from meal_pack mp
