-- Reverses 0019_create_checkout_safeupdate.sql by restoring `0014`'s body verbatim.
--
-- **This reinstates a function that fails on any hosted Supabase project**: the unqualified
-- `delete from tmp_checkout_lines;` comes back, and with it `21000: DELETE requires a WHERE
-- clause` on every order placed through PostgREST. Local `supabase start` does not load
-- `safeupdate`, so it will look fine here and be broken there — which is the whole reason
-- `0019` exists. Saying so is the point of writing the reversal out rather than marking it
-- irreversible.
