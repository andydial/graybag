-- Rollback for 0045 — production can once again take money it cannot invoice.
--
-- Worth stating plainly, because the failure it re-enables is the worst sequence in the product:
-- under auto-capture the customer is charged, `settle_payment` cannot allocate an invoice number
-- against a placeholder GSTIN, the settlement transaction rolls back, `PY2` returns 200, and the
-- sweep retries for ever. Every customer charged, no order created, no 5xx and no alert.
--
-- `create_checkout` is restored to exactly what `0039` left, taken from `pg_get_functiondef()`
-- before `0045` replaced it rather than retyped.
...
;
drop function if exists assert_seller_identity_configured();
drop function if exists seller_identity_placeholders();
