...
| RH5 | **`apps/web/public/img/` is the one narrow exception, and it is build *output* rather than source** | Added in `E12`. `RH1` keeps 46 MB of undiffable sources out of git and still holds. But Netlify's CI checkout can reach neither `../Legacy-Application/` nor `~/graybag-dish-images`, so **the site cannot build at all** without something committed. What goes in is 232 KB of re-encoded, budgeted output — 28 dish tiles at 120px, two logo lockups, one pattern, a favicon and a social card — written only by `apps/web/scripts/build-web-assets.mjs`, recorded with provenance and byte counts in `MANIFEST.json`, and held under a budget asserted by `assets.test.ts`. The sources stay outside git; what is committed is regenerable from a committed manifest. An exception without a boundary is a reversal, so the boundary is the test |
