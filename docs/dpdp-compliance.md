...
| `recipient` — tier-P columns | **Immediately** on removal — when the last active `guardian_link` is revoked | `anonymise` | **DECIDED 2026-08-11 (Andy), published in privacy policy notice version 2 §4.** The proposed 12-month tail is gone: a child's name has no statutory basis for being kept, so there is nothing for a tail to serve. Implemented in `0026` inside `deactivate_recipient`, so the erasure is in the same transaction as the removal rather than waiting for a purge job that does not exist yet (`E20-19`) | no |
