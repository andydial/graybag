...
> **SUPERSEDED 2026-08-11.** This was the `E20-24` lawyer-*template* draft, written before the
> real lawyer-drafted policy existed. Andy supplied the approved baseline on 2026-08-11
> (`docs/legal/privacy-policy-baseline.md`), and the published document is now `docs/privacy-policy.md` — baseline plus
> three tracked changes. This file is kept because its DPDP detail, its processor register and
> its grievance block are reference material for future revisions, and because deleting work is
> not the same as superseding it. **It is not published and not authoritative.**
