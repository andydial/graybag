...
| Q | Assumed for now | Blocks |
|---|---|---|
| ~~**`DM-12`** What makes parental consent "verifiable" under DPDP…~~ **DECIDED 2026-08-10 (Andy).** Capture consent at child creation in v1; do not defer it. It was one question read as two: *recording* consent is required by every answer and was never blocked, while *verifying the adult is the parent* is the legal half and is now one swappable function, `parental_verification_method()`. Decisions `C10`–`C13`, migration `0015`. **The legal half remains open as `E20-01`** — it now changes one function rather than the flow | Decided. v1 bar is `authenticated_account_holder` | ~~`E05-01`~~ built; `E20-01` still open |
...
| ~~**`[DP-10]`**~~ **DECIDED 2026-08-10 (Andy): (b), nothing to do.** Production has no users and **will not open before `E20-01` returns**; Andy tells us if that changes rather than us discovering it. The version-and-hash approach is confirmed as the right one — `E20-12`'s approved wording becomes version `2` and no existing consent is invalidated. Option (c) stays on the shelf as the cheap lever if the premise ever changes. Original question and options below | **Production must not collect consent against a notice no lawyer has read.** `0015` publishes `child_data_notice` version `1` — written for a parent, deliberately short, and **not reviewed** (`C13`). Every consent captured against it records `policy_version_id` and `content_sha256`, so it is an honest record of what was actually shown, and `E20-12` publishing approved wording as version `2` does not retroactively alter it. What is *not* handled is the gap in between: if `0015` reaches production before `E20-01` returns, real parents consent to unreviewed wording. **Options:** (a) hold `0015` out of production until `E20-12` — needs a per-environment migration gate, which does not exist and would be new machinery; (b) let it apply everywhere but keep production's first real signup behind `E20-12` — this is already true, since production has no users; (c) publish version `1` unpublished (`published_at is null`) in production only, so `create_recipient` refuses with `no_notice_published` until approved wording lands. **Recommended: (b)**, with (c) as the cheap belt-and-braces if production opens to users before the lawyer returns. Staging is unaffected and is where `E05-01` gets exercised | (b) | `E20-01`, `E20-12`, `E05-01` |

### Already open elsewhere, and load-bearing for all of the above

| Q | Why it matters here |
|---|---|
| `[DM-12]` | ~~**The biggest one in the project's compliance surface.** …**The consent UI must not be built until it returns**~~ — **decided 2026-08-10**, see the row above and `C10`–`C13`. What is left of it is `E20-01`'s verification bar, which now costs one function (`parental_verification_method()`) rather than a flow change |
...
| Q | Question | Written as | Blocks |
|---|---|---|---|
| **`[WEB-01]`** | **Photography is the biggest single upgrade this page can get, and it is also an app problem.** Every dish photograph the company owns is 120 × 120 px, verified against the source CDN with both `?w=` and the Cloudflare resize path — there is no larger original (`docs/learnings.md`, 2026-08-11). The site is designed around that honestly: a mosaic of tiles capped at 88 CSS px, nothing upscaled. But `ux-spec.md` §5.5 specifies the **app's** dish card as a 16:10 photo across half a 390pt screen, which is ~350 × 218 device pixels on a 2× phone, and a 120px source cannot fill it. So this is not a marketing nicety: **either new photography is commissioned, or the app's dish card needs a treatment that works at 120px.** A shoot also fixes `E16-29`'s three permanent 403s. | Site built around 120px, honestly. Nothing upscaled | `E12-01` is done either way; the app's card treatment is not decided |
| **`[WEB-02]`** | **May Amity International, Gem Public and Paragon Senior Secondary be named publicly?** They are named on the site today under the softer line "already serving schools across Mohali" rather than as a client list, on Andy's instruction. Schools commonly want to approve being used as a reference, and the strongest version of that section names them outright. Needs each school's agreement — and, if any of the three has lapsed, needs the name removed. | Named smaller, under a softer claim | `E12-10`, the DNS cutover. This must be settled **before** the site is public |
| **`[WEB-03]`** | **Where do enquiries go?** `ENQUIRY_NOTIFY_EMAIL` is unset. Rows will land in the `enquiry` table, but nobody is told. For the first weeks after launch that is the difference between a lead and a lead going cold. Needs one address. | Unset, and flagged | The enquiry form is only half-useful until this is answered |
| **`[WEB-04]`** | **What may we say about food safety?** The site's FAQ answers the hygiene question structurally — everything is cooked to the morning's order list, nothing is stored at the school, nothing sits in a display case — and deliberately claims **no licence, no certification and no FSSAI number**, because none has been evidenced to this repository. A food business selling to schools will be asked, and a licence number on the page is worth more than a paragraph. Needs the FSSAI registration details, and confirmation of what may be stated. | No claim made | Nothing, but it weakens the strongest objection a principal has |
| **`[WEB-05]`** | **The three policy documents are still drafts and cannot be published.** All three carry `«…-PENDING-…»` tokens (17 in the privacy notice, 11 in the terms, 7 in the refund policy) and a "DO NOT PUBLISH AS-IS" banner. The site builds and links them, and a **production build containing any token fails** (`E20-22`, implemented in `apps/web/src/lib/policy.ts`). So the DNS cutover cannot happen until `E20-01` returns them from a lawyer. This is not a new question — it is `E20-01` — but it is recorded here because `E12-10` now depends on it and did not obviously before. | Gate implemented; pages render a pre-launch notice | **`E12-10`.** The site cannot go live at graybag.com with placeholder law on it |
