...
export interface PackRow {
  ...
  itemsTotal: number;
  /** What the PARENT has left — valued items plus any bonus items. What the balance screen shows. */
  itemsRemaining: number;
  /**
   * Of those, how many the MONEY bought. `meal_pack.valued_remaining`.
   *
   * **Added by MOBILE on merge (`E21-78`), because without it this file computes the wrong
   * number.** Bonus items carry no value (`M12`), so they are absent from the deferred balance —
   * and `itemsRemaining` includes them. A pack that has earned its bonus and has 2 bonus items
   * left owes **nothing**; computing `price × 2 / 22` says it owes ₹272.73, and `reconcile`
   * would report that as a divergence against the ledger on every bonus-earning pack.
   *
   * It cannot be derived from the other fields either: after a cancellation returns a valued item
   * to a granted pack, the split is genuinely unknown from totals alone. So it is read.
   */
...
/**
 * What we still owe this pack, in money. `M10`, and the same arithmetic the server uses.
 *
 * **`valuedRemaining / itemsOriginal`, never `itemsRemaining / itemsTotal`.** Both denominators
 * and both numerators differ once a bonus is granted, and the difference is the whole of `M12`:
 * bonus items are a giveaway, so they are worth nothing here and `itemsOriginal` never grows.
 *
 * This must stay identical to `meal_pack_deferred_paise()` in `0086`, which is what the ledger
 * postings are computed from. `meal_pack_money.deferred_paise` exposes the server's own answer if
 * you would rather not hold a second copy — and if these two ever disagree, `reconcile()` reports
 * a divergence that is this function's fault rather than the ledger's.
 */
export function deferredPaise(
  pack: Pick<PackRow, 'pricePaidPaise' | 'valuedRemaining' | 'itemsOriginal'>,
): number {
  return halfUp(pack.pricePaidPaise * pack.valuedRemaining, pack.itemsOriginal);
}
